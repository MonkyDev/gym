        ...
        <div class="col">
            <div class="card">
                <div class="card-header">
                    <h3>Registrar Gastos de Dinero</h3>
                </div>
                <div class="card-body">
                    {!! Form::open(['route' => 'gastos.store']) !!}
                        
                        @include('administracion.gastos.partials.form')
            ...
            <br>
             @if ($errors->any())
                @include('administracion.gastos.partials.error')
            @endif
        </div>
        @endcan
        <div class="col">
            <div class="card">
                <div class="card-header">
                	<h3> 
                		Control de gastos del Diario
                		{{-- @can('gastos.create')
                		<a href="{{route('gastos.create')}}" class="btn btn-outline-primary btn-sm float-right"><i class="fas fa-dollar-sign"></i>&nbsp;Registrar</a>
                		@endcan --}}
                	</h3>
                	{{ Form::model(Request::all(), ['route'=>'gastos.index', 'method'=>'PUT']) }}
                	<div class="input-group mb-3">
					    <input type="date" name="date_search" class="form-control">
					       <div class="input-group-append">
    					    ...
    					    <button class="btn btn-outline-danger" onclick="javascript: location.href='{{ route('gastos.index') }}'" id="button-addon2"><i class="fas fa-times"></i></button>
					    </div>
					</div>
                	{{ Form::close() }}
                </div>
			    ...
			    @include('administracion.gastos.partials.table')
                @if( session('code') )
			        @include('administracion.gastos.partials.info')
			    @endif
                </div>
            </div>
        </div>
